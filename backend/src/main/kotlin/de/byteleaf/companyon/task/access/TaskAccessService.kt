...
class TaskAccessService {
    ...
    @PostAuthorize("hasPermission(T(de.byteleaf.companyon.auth.permission.PermissionType).CURRENT_USER_OR_ADMIN, returnObject.user)")
    fun get(id: String): Task = taskService.get(id)

    @PostAuthorize("hasPermission(T(de.byteleaf.companyon.auth.permission.PermissionType).CURRENT_USER_OR_ADMIN, returnObject.user)")
    fun getNullable(id: String?): Optional<Task> = taskService.getNullable(id)

    @PreAuthorize("hasPermission(T(de.byteleaf.companyon.auth.permission.PermissionType).CURRENT_USER_OR_ADMIN, #user)")
    ...
    private fun deleteWithUser(id: String, @Suppress("UNUSED_PARAMETER") userId: String): Task = taskService.delete(id)
